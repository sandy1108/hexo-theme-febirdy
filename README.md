...
已导入本机博客正在使用的 Aomori 源码、预构建资源与原始 MIT 许可证，建立独立 master 仓库和隔离生成工具。当前已完成首页、文章详情、归档、分类/标签列表的 FEBIRDY 视觉迁移，并提供分类/标签总览、关于页、404 和搜索弹层模板；没有线上切换或远端发布。
...
1. 继续细化桌面宽屏的首页比例、移动端导航与文章目录交互。
2. 根据真实博客页面回归，核对 Giscus、目录、代码高亮、图片查看等功能。
3. 配置 Algolia 后回归搜索弹层的命中、无结果和键盘操作状态。
4. 真实内容验收完成后，再评估 Aomori 可选模块的针对性清理。
